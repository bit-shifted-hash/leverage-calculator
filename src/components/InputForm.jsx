  ...
  return (
    <div className="rounded-2xl border border-slate-800 bg-slate-900/60 backdrop-blur-sm p-6 sm:p-8">
      <div className="flex items-center gap-2 mb-6">
        ...
        <h2 className="text-sm font-semibold text-slate-300 tracking-wider uppercase">参数输入</h2>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {/* 股票代码 */}
          <div>
            <label className="block text-xs text-slate-400 mb-2 tracking-wide">股票 / ETF 代码</label>
            <input
              type="text"
              value={ticker}
              ...
              placeholder="如：QQQ"
              maxLength={10}
              className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-sm text-white placeholder-slate-600 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/40 transition-all"
            />
          </div>

          {/* 总本金 */}
          <div>
            <label className="block text-xs text-slate-400 mb-2 tracking-wide">总本金 (USD)</label>
            <div className="relative">
              ...
              <input
                ...
                min="1"
                step="any"
                className="w-full bg-slate-800 border border-slate-700 rounded-xl pl-8 pr-4 py-3 text-sm text-white placeholder-slate-600 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/40 transition-all"
              />
            </div>
          </div>
          ...
          {/* A防区被套保底价 */}
          <div>
            <label className="block text-xs text-slate-400 mb-2 tracking-wide">A防区被套保底价 (USD)</label>
            <div className="relative">
              <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 text-sm">$</span>
              <input
                type="number"
